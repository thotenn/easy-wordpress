...
#TODO: ejecutar todo por separado, que los servicios del compose por ej webserver y certbot sea opcional por medio de USE_WEBSERVER y en caso de que sea true el SSL tambien opcional
...
echo "alias wp-monitor='$APP_PATH/monitor.sh'" >> /etc/bash.bashrc
echo "alias wp-restart='$APP_PATH/restart.sh'" >> /etc/bash.bashrc
